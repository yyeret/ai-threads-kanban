...
1. Parse the argument. The **first word** is the target stage; any remaining
...
2. Run this from the **current working directory** — do NOT cd anywhere.
   ...
   Add `--note "<note>"` only if note words were supplied.
3. The script auto-scans native harness history first (registering a brand-new
   thread), then re-renders the board. Report which thread was updated and its
   new stage. `done` also archives the thread.
